...
(function () {
  ...
  const LAYER = 5;     // depth layers per sample point
  ...
  function drawGitHub(s) {
    const cv = mc(s), ctx = cv.getContext('2d');
    fill(ctx, s, '#000');
    const cx=s/2, cy=s*0.44, R=s*0.26;
    ctx.strokeStyle='#fff'; ctx.fillStyle='#fff'; ctx.lineWidth=s*0.09; ctx.lineCap='round';
    ctx.beginPath(); ctx.arc(cx,cy,R,0,Math.PI*2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx-R*0.6,cy-R*0.7); ctx.lineTo(cx-R*1.0,cy-R*1.3); ctx.lineTo(cx-R*0.2,cy-R*0.9); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.moveTo(cx+R*0.6,cy-R*0.7); ctx.lineTo(cx+R*1.0,cy-R*1.3); ctx.lineTo(cx+R*0.2,cy-R*0.9); ctx.closePath(); ctx.fill();
    ctx.beginPath(); ctx.arc(cx,cy+R*0.5,R*0.58,Math.PI,0); ctx.lineWidth=s*0.08; ctx.stroke();
    return px(cv,s);
  }
  ...
  function drawLinkedIn(s) {
    ...
    ctx.lineTo(pad,pad+r); ctx.arcTo(pad,pad,pad+r,pad,r);
    ctx.closePath(); ctx.stroke();
    ctx.fillStyle='#fff'; ctx.font=`bold ${Math.round(s*0.40)}px Arial`;
